# PowerShell for Microsoft 365
PowerShell scripts and insights for Microsoft 365 services like SharePoint, Teams, etc. View the Readme file within each folder to get more detail on the functionality of each script.

-----

## Get SharePoint Site Users
Retrieves and lists all users and security groups with access to a SharePoint site

## List All SharePoint Site Files
Retrieves a complete list of all files across provided SharePoint sites. It helps inventory and audit file storage.

## Reset SharePoint Library Permissions
Resets permissions on SharePoint files in a document library to inherit from the library.
